...
public class AuthRestAPIs {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginForm loginRequest) {
		...
		List<Utilisateur> p =userRepository.findByUsername((loginRequest.getUsername()));
		return ResponseEntity.ok(new JwtResponse(jwt,p.get(0).getId(), p.get(0).getUsername(), userDetails.getAuthorities()));
	}
